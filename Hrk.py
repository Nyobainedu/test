# Credits: @tiararosebiezetta @huzunluartemis
import time, os, heroku3, requests, datetime


def changeDyno(api, name, breakdowns=0):
    try:
        heroku_conn = heroku3.from_key(api)
        app = heroku_conn.app(name)
        proclist = app.process_formation()
        for p in proclist: app.process_formation()[p.type].scale(breakdowns)
        return True
...
try:
    ...
    print("Your CONFIG_FILE_URL was empty.")
    exit(1)
